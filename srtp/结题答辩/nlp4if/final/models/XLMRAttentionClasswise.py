...
class XLMRAttentionClasswise(nn.Module):
    # TODO : Add dropout prob to arparse
    def __init__(self, freeze_bert_params=True, dropout_prob=0.1, num_heads=3, base='xlm-roberta-base'):
      print("XLMRAttentionClasswise Being Used!\n\n\n")

      super(XLMRAttentionClasswise, self).__init__()
      self.embeddings = XLMRobertaModel.from_pretrained(base)
    ...
    def forward(self, sent_id, mask, get_embedding=False, get_attn_wt=False):
      # Bert
      bert_output = self.embeddings(sent_id, attention_mask=mask, output_hidden_states=True)[1]
